...
ProbeFiles = dir('C:\Users\che7oz\Desktop\1x2x3x6x 2493 9312 bcd mrna\2493O\*_c4.jpg');
for i=1:length(ProbeFiles)
ProbeI=imread(ProbeFiles(i).name);
ProbeI=ProbeI(:,:,1);
DapiFiles = dir('C:\Users\che7oz\Desktop\1x2x3x6x 2493 9312 bcd mrna\2493O\*_c3.jpg');
...
%        figure,imshow(W);
    ...
    theta2= 0 %180*double(max(max(ProbeI1(:,1:50)))<max(max(ProbeI1(:,max(m0)-min(m0)-48:max(m0)-min(m0)+1)))); % Anterior-posterior
...
%% Specific Signal
    Diverge = im2bw(ProbeI1, graythresh(ProbeI1(ProbeI1~=0)));
